...
#### left off at trying to filter for 1312 too
...
all.data <- read_csv("Environmental_data/Master_Fragment.csv") %>%
  ...
  subset(Timepoint == "30 hour" | Timepoint == "2 week" | Timepoint == "4 week" | 
           Timepoint == "8 week" | Timepoint == "12 week") %>%
  unite(NCBI_Sample_Name, Plug_ID, Treatment, Timepoint, sep = "_", remove = FALSE)

all.data$Project <- "HoloInt_WGBS"

all.data <- all.data %>% unite(NCBI_Sample_Title, NCBI_Sample_Name, Project, sep = "_", remove = FALSE) 

write_csv(file = "~/MyProjects/Acclim_Dynamics_molecular/WGBS/NCBI-sample-title.csv", all.data)
